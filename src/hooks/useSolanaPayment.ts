...
export const useSolanaPayment = ({
  ...
  const MERCHANT_WALLET_ADDRESS = "8KzFk7cCzNXEdZHQcq9rvHdnMcqx6BPR1dG3xYUNdeaG"; // Replace with your Solana address
  ...
  const initiatePayment = async (amount: string, tx_type?: string, gif_id?: number) => {
    ...
    try {
      ...
      console.log("Using wallet:", wallet.walletClientType, "Address:", walletAddress);
        ...
        if (wallet.walletClientType === 'phantom') {
          if (window.solana) {
            ...
            try {
              ...
              txSignature = await window.solana.signAndSendTransaction(transaction);
            } catch (err) {
              console.error("Error with Phantom window.solana:", err);
            }
          } else if (window.phantom?.solana) {
            console.log("Using Phantom-specific method with window.phantom.solana");
            try {
      ...
      try {
        ...
        console.warn("Confirmation error (but transaction may still succeed):", confErr);
      }

      // Notify the backend about the deposit
      await verifyTransaction(txSignature, amount, tx_type, gif_id);
